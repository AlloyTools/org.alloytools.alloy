	...
	private Solution command(Module module, SolverOptions optionsOrNull, Command command) {
		...
		return new Solution() {
				...
				return new Iterator<Instance>() {
					...
					public Instance next() {
						...
						return new Instance() {
							...
							public Object eval(String cmd) {
								try {
									...
									if ( eval instanceof A4TupleSet) {
										return to((A4TupleSet) solution.eval(expr));
									}
							...
							@Override
							public Optional<Instance> init() {
								// TODO Auto-generated method stub
								return null;
							}

							@Override
							public Optional<Instance> config() {
								// TODO Auto-generated method stub
								return null;
							}

							@Override
							public Optional<Instance> parent() {
								// TODO Auto-generated method stub
								return null;
							}
			...
			public boolean hasVariables() {
				return getModule().getSigs().stream().flatMap(sig -> sig.getFields().stream())
						.filter(field -> field.isVariable()).findAny().isPresent();
			}
