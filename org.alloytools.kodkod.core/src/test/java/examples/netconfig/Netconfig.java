...
public class Netconfig {
	...
	public Formula traces() {
		...
		final Formula f = addSatelliteLink(r1, r2, t).or(addLineOfSightLink(r1, r2, t))
				.or(continuedConnection(r1, r2, t))
				.or(lostConnection(r1, r2, t));
	 ...
	 */
	public Formula show() {
		return declarations().and(invariants()).and(subsConnectedToHQ(end)).and(connectedSites(Site, end)).and(
				traces());
	}

	/**
	 * Returns a bounds object that constructs the 'scope' for analyzing the
	 * commands, using the given values for the number of sites, routers, and
	 * the length of time.
	 * 
