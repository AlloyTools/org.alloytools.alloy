...
public class GEO158 {
	...
	/*
	 * part_of : C -> C incident_c : P -> C sum : C -> C -> one C end_point : P
	 * -> C inner_point : P -> C meet : P -> C -> C closed : C open : C
	 */
	...
	public final Formula c2() {
		...
		return f0.and(f1).implies(f2.or(f3).or(f4)).forAll(
				c.oneOf(curve).and(c1.oneOf(curve)).and(c2.oneOf(curve)).and(c3.oneOf(curve)));
	}
	...
	public final Formula c7() {
		...
		return f0.and(f1).implies(f2).forAll(
				c.oneOf(curve).and(c1.oneOf(curve)).and(c2.oneOf(curve)).and(p.oneOf(point)));
	}
