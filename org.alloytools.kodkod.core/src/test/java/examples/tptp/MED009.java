...
public final class MED009 extends MED001 {
	...
	public final Formula transsls2_qige27() {
		...
		final Formula f0 = n0.in(s1).and(n0.join(gt).in(conditionhyper)).and(n0.in(bcapacitysn).not()).and(
				n0.in(qilt27).not());
		final Formula f1 = n0.product(x0)
				.in(gt)
				.not()
				.and(x0.in(s2))
				.and(x0.join(gt).in(conditionhyper))
				.and(x0.in(bcapacityne.union(bcapacityex)))
