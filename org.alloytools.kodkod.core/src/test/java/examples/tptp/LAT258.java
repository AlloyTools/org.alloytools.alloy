...
public final class LAT258 {
	...
	public final Formula decls() {
		return p.one().and(t.one()).and(v.one()).and(w.one()).and(u.one()).and(x.one()).and(y.one()).and(z.one());
	}
	...
	/*
	 * fof(associativity_meet,axiom,( ! [A,B,C,D,E,F] : ( ( meet(A,B,D) &
	 * meet(D,C,E) & meet(B,C,F) ) => meet(A,F,E) ) )).
	 */
	/**
	...
	public final Formula axioms() {
		return decls().and(joinAssumption())
				.and(meetAssumption())
				.and(meetJoinAssumption())
				.and(goalAx())
				.and(lessThanReflexive())
				.and(lessThanTransitive())
				.and(lowerBoundMeet())
				.and(greatestLowerBoundMeet())
				.and(upperBoundJoin())
				.and(leastUpperBoundJoin())
				.and(lessThanMeetJoin())
				.and(commutativityMeet())
				.and(commutativityJoin())
				.and(associativityMeet())
				.and(associativityJoin())
				.and(loLeDistr())
				.and(doLattice());
	}
