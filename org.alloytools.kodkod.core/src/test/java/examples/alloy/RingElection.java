...
public final class RingElection {
	...
	public Formula progress() {
		...
		final Expression t2 = t1.join(tord);
		final Variable p = Variable.unary("p");
		final Formula f1 = Process.join(toSend).join(t1).some().implies(
				skip(t1, t2, p).not().forSome(p.oneOf(Process)));
	...
	/**
	 * Returns a bounds object that scopes Process, Time, and their fields
