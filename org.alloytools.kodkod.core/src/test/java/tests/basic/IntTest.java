...
public class IntTest extends TestCase {
	 ...
	 * @requires vals contains int expressions that represent all integers
	 *           allowed by this.solver.options, in proper sequence
	 */
	...
	private final void testCompOp(IntCompOperator op, IntExpression ei, IntExpression ej, int i, int j,
			boolean result) {
	...
	private void testIfIntExpr(Options.IntEncoding encoding) {
		...
		f = ((r1.count().eq(IntConstant.constant(2))).thenElse(r1.sum(), IntConstant.constant(5)))
				.eq(IntConstant.constant(4));
		s = solve(f);
		assertNotNull(s.instance());
