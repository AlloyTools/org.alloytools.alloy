     ...
     */
     ...
     public void testTh2() {
         {
             ...
             Formula post = amb.gt(ZERO)
                       .and(amb.gt(as))
                       .and(amb.gt(bs.negate()));
             checkTrue(pre, post, decls);
         }
         {
             Formula pre = as.lt(ZERO).and(bs.gt(ZERO));
             IntExpression amb = as.minus(bs);
             Formula post = amb.lt(ZERO)
                       .and(amb.lt(as))
                       .and(amb.lt(bs.negate()));
             checkTrue(pre, post, decls);
         }
     }
     ...
     public void testTh3() {
         ...
         }
         {
             ...
             IntExpression atb = as.multiply(bs);
             Formula post = atb.gt(ZERO)
                       .and(atb.gte(as.negate()))
                       .and(atb.gte(bs.negate()));
             checkTrue(pre, post, decls);
         }
     }
     ...
     public void testTh4() {
         {
             Formula pre = as.gt(ZERO).and(bs.lt(ZERO));
             IntExpression atb = as.multiply(bs);
             Formula post = atb.lt(ZERO)
                       .and(atb.negate().gte(as))
                       .and(atb.negate().gte(bs.negate()));
             checkTrue(pre, post, decls);
         }
         {
             Formula pre = as.lt(ZERO).and(bs.gt(ZERO));
             IntExpression atb = as.multiply(bs);
             Formula post = atb.lt(ZERO)
                       .and(atb.negate().gte(as.negate()))
                       .and(atb.negate().gte(bs));
             checkTrue(pre, post, decls);
         }
     }
     ...
     /**
      * UNSAT: all a: Int | some b: Int | b = MAXINT && (a = MAXINT => (a + b = a + b))
      * SAT:   some a: Int | all b: Int | b = MAXINT && (a = MAXINT => (a + b = a + b))
      */
      ...
      * SAT: !(some a: Int | some b: Int | !(b = MAXINT => (a = MAXINT => a+a = b+b))
      */
     public void testSat() {
