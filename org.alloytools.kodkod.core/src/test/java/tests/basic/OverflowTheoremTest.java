...
public class OverflowTheoremTest extends TestCase {
	...
	/**
	 * all decl | pre => post !!(all decl | pre => post) all decl | !!(pre =>
	 * post) all decl | !(pre && !post) all decl | !pre || post !(some decl |
	 * !(pre => post)) !!!(some decl | !(pre => post)) !(some decl | pre &&
	...
	/**
	 * all a, b: Int | a > 0 && b < 0 => a * b < 0 && -(a * b) > a && -(a * b) >
	 * -b all a, b: Int | a < 0 && b > 0 => a * b < 0 && -(a * b) > -a && -(a *
	 * b) > b
	 */
	@Test
	public void testTh4() {
	 ...
	 * = b+b) UNSAT: !(all b: Int | some a: Int | !(b = MAXINT && (a = MAXINT =>
	 * a+a = b+b))) ---- UNSAT: all x: Int | all a: Int | some b: Int | b =
	 * MAXINT && (a = MAXINT => a+a = b+b) UNSAT !(some x: Int | some a: Int |
	 * all b: Int | !(b = MAXINT && (a = MAXINT => a+a = b+b))) UNSAT: all x:
	 * Int | some b: Int | all a: Int | b = MAXINT && (a = MAXINT => a+a = b+b)
	 * UNSAT: !(some x: Int | all b: Int | some a: Int | !(b = MAXINT && (a =
	 * MAXINT => a+a = b+b)))
	 */
	public void testUnsat() {
		...
		checkUnsat(body.forSome(b.oneOf(Expression.INTS)).forAll(a.oneOf(Expression.INTS)).forAll(
				x.oneOf(Expression.INTS)));
		checkUnsat(body.not()
				.forAll(b.oneOf(Expression.INTS))
				.forSome(a.oneOf(Expression.INTS))
				.forSome(x.oneOf(Expression.INTS))
				.not());

		checkUnsat(body.forAll(a.oneOf(Expression.INTS)).forSome(b.oneOf(Expression.INTS)).forAll(
				x.oneOf(Expression.INTS)));
		checkUnsat(body.not()
				.forSome(a.oneOf(Expression.INTS))
				.forAll(b.oneOf(Expression.INTS))
	 ...
	 * + b)) SAT: some a: Int | all b: Int | b = MAXINT && (a = MAXINT => (a + b
	 * = a + b))
	 */
	...
	/**
	 * SAT: all a: Int | all b: Int | b = MAXINT => (a = MAXINT => a+a = b+b)
	 * SAT: !(some a: Int | some b: Int | !(b = MAXINT => (a = MAXINT => a+a =
	 * b+b))
	 */
	public void testSat() {
