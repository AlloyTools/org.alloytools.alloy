...
public class SkolemizationTest extends TestCase {
	...
	private final void testSkolems(Multiplicity mult) {
		...
		inst = solve(va.in(r1b.join(r2b)).forSome(da).and(
				va.in(r1b).not().forAll(mult == Multiplicity.ONE ? da : va.oneOf(r1a))));
		assertSkolems(bounds, inst, skolems);
	...
	private final void testDeepSkolems(Multiplicity mult) {
		...
		inst = solve(
				va.in(vb.join(r2b).union(vd.join(r2b)).union(vc)).forAll(dc).forAll(db).forSome(da1).not().forAll(dd1));
