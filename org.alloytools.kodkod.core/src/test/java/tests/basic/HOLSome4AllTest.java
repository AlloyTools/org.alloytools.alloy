...
public class HOLSome4AllTest extends TestCase {
    ...
    public void testE3() {
        ...
        Formula cnd = si.gt(I0);
        Formula f = cnd.and(ns.some().implies(ns.count().gt(si)).forAll(ns.setOf(Node))).forSome(s.oneOf(Relation.INTS));
        Solution sol = solve(f);
        assertTrue(sol.sat());
    ...
    @Test
    public void testE4() {
        // UNSAT: some Node && some s: ints | s > 0 && (all ns: set Node | some ns => #ns > s)
        Formula cnd = si.gt(I0);
        Formula f = Node.some().and(cnd.and(ns.some().implies(ns.count().gt(si)).forAll(ns.setOf(Node))).forSome(s.oneOf(Relation.INTS)));
        Solution sol = solve(f);
        assertFalse(sol.sat());
    }
    ...
    @Test
    public void testE5() {
        // SAT: some Node && some s: ints | s >= 0 && (all ns: set Node | some ns => #ns > s)
