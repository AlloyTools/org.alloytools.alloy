...
public class BugTests extends TestCase {
	...
	public final void testFelix_03162009() {
		...
		TupleSet approx = factory.setOf(test.arity(),
				Translator.approximate(test, bounds, solver.options()).denseIndices());
	...
	public final void testFelix_05192007() {
		...
		List<String> atomlist = Arrays.asList("de[0]", "de[1]", "de[2]", "de[3]", "dir[0]", "dir[1]", "dir[2]",
				"dir[3]");
	...
	public final void testFelix_08142006() {
		...
		String[] atoms = {
				"-1", "-2", "-3", "-4", "-5", "-6", "-7", "-8", "0", "1", "2", "3", "4", "5", "6", "7", "Bool.0",
				"Bool.1", "Msg.0", "MsgState.0", "Node.0", "NodeState.0", "NodeState.1", "Ord.0", "Ord.1", "Tick.0",
				"Tick.1", "Tick.2"
		};
	...
	public final void testVincent_03182006_reduced() {
		...
		b.bound(prereqs, f.setOf(f.tuple("6.002", "[8.02]"), f.tuple("8.02", "[]"), f.tuple("6.003", "[6.001]"),
				f.tuple("6.001", "[]")));
	...
	public final void testVincent_03182006() {
		...
		b.bound(prereqsetUsed, f.setOf(f.tuple("6.002", "[8.02]"), f.tuple("8.02", "[]"), f.tuple("6.003", "[6.001]"),
				f.tuple("6.001", "[]")));
		...
		b.bound(sCourses, f.range(f.tuple("Spring 2006"), f.tuple("Fall 2006"))
				.product(f.range(f.tuple("6.002"), f.tuple("6.001"))));
		b.boundExactly(prev, f.setOf(f.tuple("Fall 2006", "Spring 2006")));
		b.boundExactly(prereqs, f.setOf(f.tuple("6.002", "[8.02]"), f.tuple("8.02", "[]"), f.tuple("6.003", "[6.001]"),
				f.tuple("6.001", "[]")));
		...
		final Variable c = Variable.unary("c");
		final Formula f7 = c.join(cAttributes).in(s.join(sAttributes)).forAll(c.oneOf(s.join(sCourses))).forAll(
				s.oneOf(Semester));
		final Variable s1 = Variable.unary("s1"), s2 = Variable.unary("s2");
		final Formula f8 = s1.join(sCourses)
				.intersection(s2.join(sCourses))
				.no()
				.forAll(s2.oneOf(Semester.difference(s1)))
				.forAll(s1.oneOf(Semester));
	...
	public final void testVincent_02182006() {
		...
		bounds.bound(course, factory.range(factory.tuple(atoms.get(courseIndex)),
				factory.tuple(atoms.get(courseIndex + courseScope - 1))));
		bounds.bound(semester, factory.range(factory.tuple(atoms.get(semesterIndex)),
				factory.tuple(atoms.get(semesterIndex + semesterScope - 1))));
		bounds.bound(prereqset, factory.range(factory.tuple(atoms.get(prereqsetIndex)),
		...
		bounds.bound(sCourses,
				factory.area(factory.tuple((Object) atoms.get(semesterIndex), (Object) atoms.get(courseIndex)),
		...
		Formula formula = (p.join(pCourses).in(s.join(prev.closure()).join(sCourses)).forAll(p.oneOf(c.join(prereqs))))
	...
	public final void testMana_01132006() {
		...
		b.bound(next,
				t.setOf(t.tuple("DblLinkedListElem0", "null"), t.tuple("DblLinkedListElem0", "DblLinkedListElem0")));
