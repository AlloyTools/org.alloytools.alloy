...
public class OverflowSigTest extends TestCase {
    ...
    /**
     * sAtoms = [S0, S1, ..., S<numS-1>]
     * xAtoms = [X0, X1, ..., S<numX-1>]
     */
    protected void createAtoms(int numS, int numX) {
        sAtoms = new ArrayList<String>(numS);
        xAtoms = new ArrayList<String>(numX);
        for (int i = 0; i < numS; i++) sAtoms.add(String.format("S%d", i));
        for (int i = 0; i < numX; i++) xAtoms.add(String.format("X%d", i));
    }
    ...
    /**
     * all v: S | <int_op>[#s.sa, #s.sb] = #s.sc
     *
     * <int_op> = {plus, minus, times, div, mod}
     */
