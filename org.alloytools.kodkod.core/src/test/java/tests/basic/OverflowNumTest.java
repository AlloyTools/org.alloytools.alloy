...
public class OverflowNumTest extends TestCase {
    ...
    public abstract class GenericTester extends Tester {
        public void doTmpTest() {
            ...
            for (IntOperator op : ops) for (int i = l; i < h; i++) for (int j = l; j < h; j++) {
                ...
                int javaResult = -1;
                try { javaResult = exeJava(op, i, j); } catch(ArithmeticException e) {continue;}
    ...
    public void testPlus() {
        runTestForAll(new GenericTester() {
            ...
            @Override protected IntExpression kodkodOpExpr(IntExpression i, IntExpression j) { return i.plus(j); }
    ...
    public void testMinus() {
        runTestForAll(new GenericTester() {
            @Override protected int exeJava(int i, int j) { return i - j; }
            @Override protected IntExpression kodkodOpExpr(IntExpression i, IntExpression j) { return i.minus(j); }
            @Override protected String getOp() { return "-"; }
        });
    }
    ...
    public void testTimes() {
        runTestForAll(new GenericTester() {
            @Override protected int exeJava(int i, int j) { return i * j; }
            @Override protected IntExpression kodkodOpExpr(IntExpression i, IntExpression j) { return i.multiply(j); }
            @Override protected String getOp() { return "*"; }
        });
    }
    ...
    public void testDiv() {
        runTestForAll(new GenericTester() {
            @Override protected int exeJava(int i, int j) { return i / j; }
            @Override protected IntExpression kodkodOpExpr(IntExpression i, IntExpression j) { return i.divide(j); }
            @Override protected String getOp() { return "/"; }
    ...
    public void testMod() {
        runTestForAll(new GenericTester() {
            @Override protected int exeJava(int i, int j) { return i % j; }
            @Override protected IntExpression kodkodOpExpr(IntExpression i, IntExpression j) { return i.modulo(j); }
            @Override protected String getOp() { return "%"; }
            @Override protected boolean skip(int i, int j) { return j == 0; }
        });
    }
    ...
    public void testTmp() {
        new GenericTester() {
            @Override protected IntExpression kodkodOpExpr(IntExpression i, IntExpression j) { return null; }
            @Override protected String getOp() { return null; }
            @Override protected int exeJava(int i, int j) { return 0; }
        }.doTmpTest();
    }
