		...
		public void doTmpTest() {
			...
			for (IntOperator op : ops)
				for (int i = l; i < h; i++)
					for (int j = l; j < h; j++) {
						Formula f = ret
								.eq(compose(op, IntConstant.constant(i), IntConstant.constant(j)).toExpression());
						int javaResult = -1;
						try {
						...
						if (javaResult >= h || javaResult < l) {
							...
							try {
								int kkResult = exeKodkod(f);
								assertEquals(String.format("Wrong result: (%s) %s (%s) != %s", i, op, j, kkResult),
										javaResult, kkResult);
							} catch (NoSol e) {
		...
		protected void doTestDef(int i, int j) {
			...
			try {
				...
				String msg = String.format(
						"Expected the default value (%d) due to overflow (%d %s %d) but got exception: %s", DEF_VAL, i,
						op, j, e.getClass().getSimpleName() + ": " + e.getMessage());
				fail(msg);
			}
		}
