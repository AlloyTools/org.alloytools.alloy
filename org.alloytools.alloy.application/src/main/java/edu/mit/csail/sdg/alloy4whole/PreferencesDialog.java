...
public class PreferencesDialog extends JFrame {
	...
	protected Iterable<SatSolver> testSolvers() {
		...
		if (now == SatSolver.SAT4J && satChoices.size() > 3 && satChoices.contains(SatSolver.CNF)
				&& satChoices.contains(SatSolver.KK)) {
	...
	protected Component initSolverPane() {
		JPanel p = OurUtil.makeGrid(2, gbc().make(), mkCombo(Solver), mkSlider(SkolemDepth), mkCombo(Unrolls),
				mkCombo(CoreGranularity), mkSlider(CoreMinimization));
		int r = 5;
