...
public class Cvc4EnumerationTask implements WorkerEngine.WorkerTask
{
    ...
    public void run(WorkerEngine.WorkerCallback workerCallback) throws Exception
    {
        try
        {
            ...
            // (check-sat)
            String result = Cvc4Task.cvc4Process.sendCommand(AbstractTranslator.CHECK_SAT);
            if(result != null)
            {
                switch (result)
                {
    ...
    {
        String smtModel     = Cvc4Task.cvc4Process.sendCommand(AbstractTranslator.GET_MODEL);
