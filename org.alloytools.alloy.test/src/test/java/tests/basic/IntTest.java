...
public class IntTest extends TestCase {
    ...
    public IntTest(String arg0) {
        ...
        List<String> atoms = new ArrayList<>(SIZE);
    ...
    private boolean overflows(IntExpression ei, int i, int realResult) {
        ...
        if (outOfBounds(realResult, bw) || (ei.toString().contains("-#") && outOfBounds(-i, bw)))
            return true;
        return false;
    }
    ...
    private boolean overflows(IntOperator op, IntExpression ei, IntExpression ej, int i, int j, int realResult) {
        if (!solver.options().noOverflow())
            return false;
        int bw = solver.options().bitwidth();
        if ((ei.toString().contains("-#") && outOfBounds(-i, bw)) || (ej.toString().contains("-#") && outOfBounds(-j, bw)))
            return true;
        if (op == SHA || op == SHR)
            return false;
        if (op == SHL) {
    ...
    public void testBitsetCast() {
        ...
        final List<Integer> atoms = new ArrayList<>(width);
        for (int i = 0; i < msb; i++) {
