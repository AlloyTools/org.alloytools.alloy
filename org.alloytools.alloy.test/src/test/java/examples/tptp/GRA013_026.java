...
/**
 * A KK encoding of GRA019+1.p through GRA026+1.p from
...
public final class GRA013_026 {
    ...
    public GRA013_026(int graphSize, int cliqueSize) {
        if (cliqueSize <= 0)
            throw new IllegalArgumentException("cliqueSize must be positive: " + cliqueSize);
        if (cliqueSize > graphSize)
            throw new IllegalArgumentException("cliqueSize must be less than or equal to graph size: " + cliqueSize + ">" + graphSize);
    ...
    private final Formula cliqueAxiom(Expression color) {
        ...
        final List<Expression> members = new ArrayList<>(cliqueSize);
        for (int i = 0, max = cliqueSize - 1; i < max; i++) {
            final List<Expression> tmp = new ArrayList<>();
    ...
    public final Bounds bounds() {
        final List<String> atoms = new ArrayList<>(graphSize);
