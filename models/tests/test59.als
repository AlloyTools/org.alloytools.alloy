...
fact {
   all p:Path {
      (no p.names) => p.target=(Dir <: iden)
      (one p.names) => p.target={ d:Dir, o:Obj | d.map[p.names.first]=o }
      (!lone p.names) => p.target={ d:Dir, o:Obj | p.parent.target[d].map[p.names.last]=o }
      (no p.names) => (no p.parent)
      (some p.names) => (one p.parent && p.names = p.parent.names + (#p.parent.names)->(p.names.last))
   }
}

// This makes more efficient use of the scope, by ensuring that any two distinct Paths will have distinct sequence of Names
fact {
