...
public class A4Preferences {
	...
	public abstract static class Pref<T> extends DefaultSingleSelectionModel {
		...
		/**
		 * Reads the value for this preference; if not set or is empty, we
		...
		/**
		 * Parses a given string value into the original type (T) of this
		 * preference.
		 */
	...
	public static class ChoicePref<T> extends Pref<T> {
		...
		/**
		 * Returns the index of the given value (choice) in the list of all
		 * valid choices
		 */
	...
	public static class DelayedChoicePref<T> extends ChoicePref<T> {
		...
		public DelayedChoicePref(String id, String title, String desc, Iterable<T> validChoices,
				T... defValueCandidates) {
	...
	public static class IntChoicePref extends ChoicePref<Integer> {
		...
		public IntChoicePref(String id, String title, String desc, Iterable<Integer> validChoices,
		...
		public static IntChoicePref range(String id, String title, String desc, int min, int step, int max,
				int defaultValue) {
	...
	public static class StringChoicePref extends ChoicePref<String> {
		...
		public StringChoicePref(String id, String title, Iterable<String> validChoices,
				String... defaultValueCandidates) {
			super(id, title, validChoices, defaultValueCandidates);
		}

		public StringChoicePref(String id, String title, String desc, Iterable<String> validChoices,
				String... defaultValueCandidates) {
	...
	public static final class StringPref extends Pref<String> {
		...
		/**
		 * Constructs a new StringPref object with the given id and the given
		 * default value.
		 */
	...
	public static final class IntPref extends Pref<Integer> {
		...
		/**
		 * If min>n, we return min; else if n>max, we return max; otherwise we
		 * return n.
		 */
		...
		/**
		 * Make a new IntPref object with the given id; you must ensure max >=
		 * min, but def does not have to be between min..max
		 */
	...
	public static final BooleanPref						AntiAlias				= new BooleanPref("AntiAlias",
			"Use anti-aliasing");
	...
	public static final BooleanPref						NoOverflow				= new BooleanPref("NoOverflow",
			"Prevent overflows", true);

	/** The latest X coordinate of the Alloy Analyzer's main window. */
	public static final IntPref							AnalyzerX				= new IntPref("AnalyzerX", 0, -1,
			65535);

	/** The latest Y coordinate of the Alloy Analyzer's main window. */
	public static final IntPref							AnalyzerY				= new IntPref("AnalyzerY", 0, -1,
			65535);

	/** The latest width of the Alloy Analyzer's main window. */
	public static final IntPref							AnalyzerWidth			= new IntPref("AnalyzerWidth", 0, -1,
			65535);

	/** The latest height of the Alloy Analyzer's main window. */
	public static final IntPref							AnalyzerHeight			= new IntPref("AnalyzerHeight", 0, -1,
			65535);

	/** The latest font size of the Alloy Analyzer. */
	public static final IntChoicePref					FontSize				= new IntChoicePref("FontSize",
			"Font size",
			Arrays.asList(9, 10, 11, 12, 14, 16, 18, 20, 22, 24, 26, 28, 32, 36, 40, 44, 48, 54, 60, 66, 72), 14);

	/** The latest font name of the Alloy Analyzer. */
	public static final StringChoicePref				FontName				= new StringChoicePref("FontName",
			"Font family",
			Arrays.asList(GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames()),
			"Lucida Grande");

	/** The latest tab distance of the Alloy Analyzer. */
	public static final IntChoicePref					TabSize					= IntChoicePref.range("TabSize",
			"Tab size", 1, 1, 16, 4);

	/** The latest welcome screen that the user has seen. */
	public static final BooleanPref						Welcome					= new BooleanPref("Welcome",
			"Show welcome message at start up");

	/** Look and feel */
	public static final StringChoicePref				LAF						= new StringChoicePref("LAF",
			"Look and feel", Arrays.asList("Native", "Cross-platform"),
			Util.onMac() || Util.onWindows() ? "Native" : "Cross-platform");

	/** Whether syntax highlighting should be disabled or not. */
	public static final BooleanPref						SyntaxDisabled			= new BooleanPref(
			"SyntaxHighlightingDisabled", "Disable syntax highlighting");

	/** The number of recursion unrolls. */
	public static final IntChoicePref					Unrolls					= new IntChoicePref("Unrolls",
			"Recursion depth", Arrays.asList(-1, 0, 1, 2, 3), -1) {
																					@Override
	...
	/** The skolem depth. */
	public static final IntChoicePref					SkolemDepth				= new IntChoicePref("SkolemDepth3",
			"Skolem depth", Arrays.asList(0, 1, 2, 3, 4), 1);
	...
	public static final IntChoicePref					CoreMinimization		= new IntChoicePref("CoreMinimization",
			"Unsat core minimization", Arrays.asList(0, 1, 2), 2) {
																					@Override
																					public Object renderValueShort(
																							Integer value) {
																						return coreMinimizationLabels[value
																								* 2];
																					}

																					@Override
																					public Object renderValueLong(
																							Integer value) {
																						return coreMinimizationLabels[value
																								* 2 + 1];
																					}
	...
	private static final String[]						coreGranularityLabels	= new String[] {
			"Top-level", "Top-level conjuncts only", "Flatten once", "Flatten the formula once at the beginning",
			"Flatten twice", "Flatten the formula at the beginning and after skolemizing", "Expand quantifiers",
			"In addition to flattening the formula twice, expand the quantifiers"
	};
	/** The unsat core granularity. */
	public static final IntChoicePref					CoreGranularity			= new IntChoicePref("CoreGranularity",
			"Unsat core granularity", Arrays.asList(0, 1, 2, 3), 0) {
																					@Override
																					public Object renderValueShort(
																							Integer value) {
																						return coreGranularityLabels[value
																								* 2];
																					}
	 ...
	 */
	public static final IntChoicePref					SubStack				= new IntChoicePref("SubStack",
			"Maximum stack", Arrays.asList(1024, 2048, 4096, 8192, 16384, 32768, 65536), 8192) {
																					@Override
																					public Object renderValueShort(
																							Integer value) {
	...
	public static final BooleanPref						InferPartialInstance	= new BooleanPref(
			"InferPartialInstance", "Infer partial instance");

	public static final DelayedChoicePref<SatSolver>	Solver					= new DelayedChoicePref<SatSolver>(
			"SatSolver2", "Solver", SatSolver.values(), SatSolver.SAT4J) {
																					@Override
	...
	public static final ChoicePref<Verbosity>			VerbosityPref			= new ChoicePref<Verbosity>("Verbosity",
			Verbosity.values(), Verbosity.DEFAULT) {
																					@Override
																					protected String serialize(
																							Verbosity value) {
																						return value.id;
