/**
* Copied from Nancy Day project under the ASL2.0 license.

The buffer module holds a few advantages over Alloy’s built-in
sequence module. There is only one index set in the sequence module meaning that
every buffer containing the same set of values must be of the same size. The buffer module
alleviates this issue by using a new index set per buffer. Furthermore, the sequence module
does not have constructs for removing an element or adding an element to the front of a
buffer. The buffer module adds constructs for removing an element, and adding an
element to the front of a buffer.
*/
